...
notkonw_port_list = [] # 全局变量，用于记录不确定的端口
...
def range_scan(source_ip, dest_ip, port) :
    ...
    if (tcp_header_ret[13]) == 0x04: # RST flags 这里取到的8位 前两位为保留值,我们希望rst值为1,也就是第6位要为1,希望得到的结果：00000100
        return
    else:
        notkonw_port_list.append(port)
...
def main():
    ...
    try:
        tgtName = socket.gethostbyaddr(ipdest)
        # print(tgtName)
    except Exception as e:
        print(e)
        return
        ...
        for i in notkonw_port_list:
            print('[-] %d/tcp not sure'%i)
